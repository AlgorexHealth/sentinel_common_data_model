...
| `PDX` | Char(1) | `P` = Principal<br>`S` = Secondary<br/>`X` = Unable to Classify<br>' ' = Missing/ Blank | Conditional on `EncType` value | Principal discharge diagnosis flag. Populate only for `IP` and `IS` encounters. For `ED`, `AV`, and `OA` encounter types, set as missing/blank. One principal diagnosis is expected per Encounter, although in some instances more than one diagnosis may be flagged as principal. | `P` |
