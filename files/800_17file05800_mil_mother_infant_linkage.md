...
| --- | --- | --- | --- | --- |--- |
| `MPatID`<sup>1</sup> | Num(#) | Unique patient identifier | Required for mother | Arbitrary person-level identifier. Used to link across tables. Use the fewest number of bytes necessary to hold all distinct values; see ["SAS Lengths" Reference Table](SAS_lengths_reference_table.md). Must match mother `PatID` value in all other SCDM tables. Blank for infant-only records. | `123456789` |
...
| `CPatID` | Num(#) | Unique member identifier. | Required for infant | Arbitrary person-level identifier. Used to link across tables. Use the fewest number of bytes necessary to hold all distinct values; see ["SAS Lengths" Reference Table](SAS_lengths_reference_table.md). Must match infant `PatID` value in all other SCDM tables. Missing for mother/delivery only records. | `12341234` |
